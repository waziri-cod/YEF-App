...
export function Navigation() {
  ...
  // Public links visible to everyone (unauthenticated users see only these)
  const publicNavItems = [
    { to: ROUTES.HOME, icon: Home, label: t("home") },
  ];

  // Authenticated links shown after login (clients and admins)
  const privateNavItems = [
    { to: ROUTES.LOANS, icon: DollarSign, label: t("loans") },
    { to: "/loan-packages", icon: DollarSign, label: "Loan Packages" },
    { to: "/marketplace", icon: ShoppingBag, label: "Marketplace" },
    { to: ROUTES.COURSES, icon: BookOpen, label: t("courses") },
    { to: ROUTES.MENTORSHIP, icon: Users, label: t("mentorship") },
    { to: ROUTES.DASHBOARD, icon: LayoutDashboard, label: t("dashboard") },
    // analytics is admin-only
    { to: ROUTES.PROFILE, icon: UserCircle, label: t("profile") },
    { to: ROUTES.SECURITY_SETTINGS, icon: Lock, label: "Security" },
  ];
  ...
  const adminNavItems = isAdmin
    ? [
        { to: ROUTES.ADMIN_DASHBOARD, icon: LayoutDashboard, label: t("adminDashboard") || "Admin" },
        { to: ROUTES.ANALYTICS, icon: BarChart3, label: "Analytics" },
      ]
    : [];

  const navItems = user ? [...publicNavItems, ...privateNavItems, ...adminNavItems] : publicNavItems;
